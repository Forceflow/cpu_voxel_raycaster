			...
			while((!t.isTerminated())){
				if(t.getCurrentNode()->isLeaf()){
					if(t.getCurrentNode()->hasData()){
						texture_array[index] = (unsigned char) tree->leafdata[t.getCurrentNode()->data].color[0];
						texture_array[index+1] = (unsigned char) tree->leafdata[t.getCurrentNode()->data].color[1];
						texture_array[index+2] = (unsigned char) tree->leafdata[t.getCurrentNode()->data].color[2];
