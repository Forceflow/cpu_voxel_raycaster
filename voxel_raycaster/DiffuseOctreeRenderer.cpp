				...
				if(t.getCurrentNode()->isLeaf() && t.getCurrentNode()->hasData() && t.stack.back().t0.max() >0){
					...
					for(int i = 0; i<rc.lights.size(); i++){
						if(rc.lights[i].active){
							...
							// Phong
							float phong_factor = pow(s DOT tree->data[t.getCurrentNode()->data].normal, rc.lights[i].SHININESS);
					...
					texture_array[index] = (unsigned char) clampf(255*(tree->data[t.getCurrentNode()->data].color[0]/250.0f + r),0,255);
					texture_array[index+1] = (unsigned char) clampf (255*(tree->data[t.getCurrentNode()->data].color[1]/250.0f + g),0,255);
					texture_array[index+2] = (unsigned char) clampf (255*(tree->data[t.getCurrentNode()->data].color[2]/250.0f + b),0,255);
